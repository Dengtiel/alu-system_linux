...
/**
 * get_section_type_char32 - get char for a symbol based on its section (32bit)
 * @shdr: pointer to section header of the symbol's section
 * @bind: symbol binding (STB_LOCAL, STB_GLOBAL, STB_WEAK)
 *
 * Return: character representing section type
 */
static char get_section_type_char32(Elf32_Shdr *shdr, unsigned char bind)
{
	...
	if (type == SHT_PROGBITS && (flags & SHF_EXECINSTR))
		...
		c = 'r';
	else if (type == SHT_NOBITS && (flags & SHF_WRITE))
		c = 'b';
 ...
 *
 * Return: character representing symbol type, 0 to skip symbol
 */
		...
		int shnum, int swap)
{
	...
	shdr = &shdrs[shndx];
	return (get_section_type_char32(shdr, bind));
}

/**
 * get_section_type_char64 - get char for a symbol based on its section (64bit)
		...
		int shnum, int swap)
{
	...
	shdr = &shdrs[shndx];
	return (get_section_type_char64(shdr, bind));
}
